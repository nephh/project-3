...
export default function DashboardCommunities({ communities, mobile }) {
  return (
    <>
      <div className="mx-5 mt-10 hidden md:block">
        <h1 className="mb-4 ml-2 text-xl font-bold text-zinc-200">
          Your Communities
        </h1>
        {/* <div className="mb-2">
        <button
      ...
      {mobile && (
        <div className="fixed bottom-32 right-6 mx-auto max-w-sm transform flex-col rounded-lg bg-zinc-900 px-4 py-6 shadow-md duration-200">
          <ul className="border-t border-zinc-400 border-opacity-25">
            {communities.map((community, index) => (
              <li
                key={index}
